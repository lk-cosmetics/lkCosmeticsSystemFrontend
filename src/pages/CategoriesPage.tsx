...
import type { WooCommerceCategoryPreviewResponse } from '@/services/category.service';
      ...
      <TableCell>
        <div className="flex items-center gap-3">
          <div className="size-10 rounded-lg overflow-hidden bg-l-bg-2 dark:bg-d-bg-2 flex items-center justify-center border border-l-border dark:border-d-border flex-shrink-0 group-hover:border-primary/30 transition-colors">
            ...
            <FolderTree
              className={`size-5 text-l-text-3 dark:text-d-text-3 ${category.image_url ? 'hidden' : ''}`}
            />
          </div>
          <div className="min-w-0">
            ...
            <p className="text-xs text-l-text-3 dark:text-d-text-3 font-mono">
              {category.slug}
            </p>
          </div>
        </div>
...
export default function CategoriesPage() {
  ...
  const [previewData, setPreviewData] =
    useState<WooCommerceCategoryPreviewResponse | null>(null);
  ...
  useEffect(() => {
    ...
    if (salesChannelFilter !== 'all') {
      filtered = filtered.filter(
        category => category.sales_channel === Number(salesChannelFilter)
      );
    }
    ...
    if (parentFilter === 'root') {
      ...
      filtered = filtered.filter(
        category => category.parent === Number(parentFilter)
      );
    }
  ...
  const handleConfirmSync = async () => {
    ...
    try {
      ...
      setSuccessMessage(
        'Categories synchronized successfully from WooCommerce!'
      );
      setSuccessDialog(true);
      setSyncDialog(false);
  ...
  const selectAllWcCategories = () => {
    if (previewData) {
      setSelectedWcCategories(previewData.categories.map(c => c.wc_id));
    }
  };
  ...
  return (
    <div className="flex flex-1 flex-col gap-6 p-4 lg:p-6">
      ...
      <div className="flex flex-col gap-4">
        ...
        <div
          className={`overflow-hidden transition-all duration-300 ease-out ${selectedCategories.length > 0 ? 'max-h-24 opacity-100' : 'max-h-0 opacity-0'}`}
        >
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
                <div className="p-3 rounded-lg bg-l-bg-2/50 dark:bg-d-bg-2/50 space-y-1">
                  ...
                  <p className="font-semibold text-primary">
                    #{selectedCategory.wc_category_id}
                  </p>
                </div>
                <div className="p-3 rounded-lg bg-l-bg-2/50 dark:bg-d-bg-2/50 space-y-1">
                  <p className="text-xs text-l-text-3 dark:text-d-text-3 uppercase tracking-wide">
                ...
                <div className="p-3 rounded-lg bg-l-bg-2/50 dark:bg-d-bg-2/50 space-y-1">
                  ...
                  <p className="font-medium truncate">
                    {selectedCategory.parent_name || 'Root'}
                  </p>
                </div>
                <div className="p-3 rounded-lg bg-l-bg-2/50 dark:bg-d-bg-2/50 space-y-1">
                  <p className="text-xs text-l-text-3 dark:text-d-text-3 uppercase tracking-wide">
          ...
          <div className="flex-1 space-y-4 max-h-[65vh] overflow-y-auto pr-2 py-4">
            ...
            <div className="space-y-2">
              ...
              <Select
                ...
                onValueChange={v => handleFormChange('parent', v)}
              >
                <SelectTrigger>
                  <Layers className="size-4 mr-2" />
            ...
            <AlertDialogDescription className="pt-2">
              ...
              {categoryToDelete && categoryToDelete.children_count > 0 && (
                <span className="block mt-3 p-2 bg-amber-50 dark:bg-amber-950/30 rounded border border-amber-200 dark:border-amber-800 text-amber-700 dark:text-amber-400 text-sm">
                  ⚠️ This category has {categoryToDelete.children_count}{' '}
                  subcategories that may be affected.
                </span>
              )}
            </AlertDialogDescription>
          </AlertDialogHeader>
      ...
      <Dialog open={previewDialog} onOpenChange={setPreviewDialog}>
        <DialogContent className="max-w-4xl max-h-[90vh]">
          <DialogHeader>
            ...
            <DialogDescription>
              Select categories to sync from WooCommerce. Categories marked
              "Exists" will be updated.
            </DialogDescription>
          </DialogHeader>
            ...
            <>
              ...
              <div className="flex gap-2 pb-2">
                ...
                <Button
                  variant="outline"
                  size="sm"
                  onClick={deselectAllWcCategories}
                >
                  Deselect All
                </Button>
                <Button
                  ...
                  <TableBody>
                    {previewData.categories.map(category => (
                      <TableRow
                        key={category.wc_id}
                        className={
                          selectedWcCategories.includes(category.wc_id)
                            ? 'bg-primary/10'
                            : ''
                        }
                      >
                        <TableCell>
                          <Checkbox
                            checked={selectedWcCategories.includes(
                              category.wc_id
                            )}
                            onCheckedChange={() =>
                              toggleWcCategorySelection(category.wc_id)
                            }
                          />
                        </TableCell>
                        <TableCell>
                        ...
                        <TableCell>
                          {category.parent_id > 0 ? (
                            <Badge variant="secondary">
                              {previewData.categories.find(
                                c => c.wc_id === category.parent_id
              ...
              <div className="flex gap-3 pt-4 border-t">
                ...
                >
                  ...
                  {syncSelectedMutation.isPending
                    ? 'Syncing...'
                    : `Sync Selected (${selectedWcCategories.length})`}
                </Button>
                <Button
